...
export default function PaymentSheet({ isOpen, onClose, onConfirmed, price = 0 }) {
  ...
  const showPaymentSuccess = useCallback(() => {
    ...
    gsap.timeline()
      .to(icon, { scale: 1, opacity: 1, duration: 0.52, ease: 'back.out(1.8)' })
      .to(text, { y: 0, opacity: 1, duration: 0.38, ease: 'power3.out' }, 0.16)
      .to({}, { duration: 1.3 })
      .to([icon, text], { y: -12, opacity: 0, duration: 0.26, ease: 'power2.in', stagger: 0.05 })
      .to(overlay, { autoAlpha: 0, duration: 0.2, ease: 'power2.in' }, '-=0.12')
