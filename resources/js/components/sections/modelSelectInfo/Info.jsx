...
export default function Info({ model }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full container pb-20">
      {/* Price options */}
      {model.price_options?.length > 0 && (
        <div className="flex flex-col gap-2.5 py-2 bg-[#EFEEF3] w-full rounded-2xl">
          {model.price_options.map((opt, i) => (
            <Row
              key={opt.id}
              label={opt.label ?? `${opt.hours} ч`}
              value={`฿ ${opt.price_thb?.toLocaleString()}`}
              last={i === model.price_options.length - 1}
            />
          ))}
        </div>
      )}
      ...
      {/* Physical info */}
      <div className="flex flex-col gap-2.5 py-2 bg-[#EFEEF3] w-full rounded-2xl">
        <Row label="Возраст"       value={model.age ? `${model.age} лет` : '—'} />
